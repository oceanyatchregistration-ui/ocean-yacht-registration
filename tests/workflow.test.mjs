...
const adminEmail='admin@example.com',adminPassword='correct-horse-battery-staple';
let adminCookie='';
...
assert.equal((await call('/api/admin/login',{method:'POST',body:{email:'stranger@example.test',password:adminPassword}})).status,401);
assert.equal((await call('/api/admin/login',{method:'POST',body:{email:adminEmail,password:'wrong-password-value'}})).status,401);
const login=await call('/api/admin/login',{method:'POST',body:{email:adminEmail,password:adminPassword}});assert.equal(login.status,200);adminCookie=login.headers.get('set-cookie').split(';')[0];assert.match(login.headers.get('set-cookie'),/HttpOnly/);assert.match(login.headers.get('set-cookie'),/SameSite=Strict/);
...
const r=await call('/api/draft/submit',{method:'POST',cookie:app.cookie,body:{version:app.data.version,consent:true,consentVersion:'oyr-registration-v1',price:1,status:'COMPLETED',customerId:'attacker'}});assert.equal(r.status,201);assert.match(r.data.reference,/^OYR-[A-Z2-9]{12}$/);assert.equal(r.data.status,'SUBMITTED');assert.equal(r.data.pricing.mode,'FIXED');assert.equal(r.data.pricing.amountMinor,61400);assert.equal(r.data.pricing.currency,'EUR');reference=r.data.reference;
...
let detail=await call('/api/admin/applications/'+reference,{cookie:adminCookie});assert.equal(detail.data.customer.name,'Workflow Test');assert.equal(detail.data.vessel.name,'Test Horizon');assert.equal(detail.data.documents.length,1);version=detail.data.version;
...
const bad=await call('/api/track',{method:'POST',body:{reference,email:'wrong@example.test'}});const missing=await call('/api/track',{method:'POST',body:{reference:'OYR-NOTFOUND1234',email:'wrong@example.test'}});assert.deepEqual(bad.data,missing.data);
